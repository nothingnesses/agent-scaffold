...
Make small, reviewable changes that satisfy the plan and the triager's valid verdicts. Keep the plan's status current as you go: per the plan's Documentation Protocol, set the step's Status cell in the Roadmap table, which is the single source of truth for status, and change the status there only, not in prose elsewhere. Do not expand scope beyond the plan and the verdicts; flag anything else rather than doing it silently.
