...
THIS PROJECT HAS RULED THE DUPLICATION A DEFECT TWICE, AND THE SECOND RULING WAS WAITING FOR EXACTLY THIS STEP. `docs/plans/agent-scaffold.ledger.md:46` records round 2 finding G1, a bundle including "repeats `next` status label", and `:62` records the triager ruling it VALID at `high`. `:99` records round 3 finding L1, "`state-schema` repeats its status label", and `:101` records all eight findings of that round valid, with L1 alone left unfixed for one stated reason: "the leading status-word is a plan-wide convention, do not single out `state-schema` (leave it)". L1 WAS RAISED WHILE THE LABEL WAS ACCURATE, since `state-schema` was `deferred` and its opening said "Deferred", and it was still valid. So the DUPLICATION is the defect, independent of whether the copy currently happens to be right, and writing 21 accurate labels would not close the class. This step is the plan-wide pass that L1's triager deferred to.
...
A PLAIN STRIP IS NOT THE FIX, MEASURED. Applying a bare token deletion to all 21 in a throwaway copy leaves seven openings that do not begin as prose: `state-schema` becomes "and non-blocking (`Q-11`); ...", `workflow-calibration` becomes "(deferred). The scaffolded workflow's ...", `instrument-flag` becomes "optional. The design is decided below ...", `reviewer-reproducible-evidence` becomes "(built first, before resuming the paused ...)", and `optional-modules`, `structured-skeleton` and `workflow-driver` each begin mid-sentence in lower case. Criterion 11 is the oracle for this and it flags exactly those seven today. Each needs an AUTHORED opening, not a strip. The other 14 strip cleanly to "Decided (...", "Scheduled by the human ..." or "This is a ...".
...
The second half is a reading, not a command: no opening line of the 21 names a Roadmap status for its own step at all, which is what `docs/plans/agent-scaffold.md:26` requires and what criterion 1's empty output cannot show.
