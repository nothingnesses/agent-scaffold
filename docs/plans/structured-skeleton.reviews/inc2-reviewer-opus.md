...
- Empty-string values: rejected at schema time (`require_structured_ids` -> `field \`step\`/\`increment\` is empty`, tests `a*round_with_an_empty_structured_step_is_reported`, `an_escalation_with_an_empty_structured_increment_is_reported`) AND defensively treated as absent by `parse*\*` (`.filter(|s| !s.is_empty())`), so a blank id can neither validate nor join to an empty slug. Consistent.
