...
```
workspace_dir = "~/agent-box/workspaces"
...
Run a writer under isolation. For the worktree lifecycle the orchestrator already owns, use the sandboxed worktree mode so a writer agent sees only committed files: create a session from the repository, then spawn the writer against it.
