...
Status: in progress; see the Roadmap for per-step status and order. In short: the core is complete (the assets and principle data, the file-dropper with two-tier ownership, the idempotency and write-safety pass, the selection UI and its polish, and bring-your-own-template support), `init-vcs` (opt-in `--init` git repository) is complete, and the remaining steps are optional. No open questions remain. The implementation lives in the repo (`src/`, `pack/`); this spec is the durable context for resuming after a compaction, and the "Repository Layout and Current Architecture" section maps the shipped code so a fresh implementor can continue without prior context. Verification convention: `cargo clippy --all-targets -- -D warnings`, `nix fmt`, and ASCII-clean before each commit.
...
An opt-in `--init` flag initialises an empty version-control repository in the output directory, git by default (design from OQ-K). Adopted decisions and behaviour:
...
- Mechanism: `run_git_init` shells out to `git init <output-dir>` (no libgit2 or Nix dependency, consistent with `git-url-fetch`). If `--init` is given but the `git` binary is not found, the run errors (exit 2) rather than silently skipping.
- Write-safety and idempotency: `init_plan` decides the action from a read only (`InitPlan::Init` / `SkipExists` / `None`), so the preview never writes. The repository line appears first in the plan (`init  git repository`, or `skip (exists)  git repository` when a `.git` entry is already present) and `git init` runs only when writing is confirmed (`--write` or the selector's Save), before the assets are dropped. An existing repository is skipped, never reinitialised (Principles 3 and 4). The repository is left empty (no initial commit); committing the scaffolded files stays the user's choice.
...
Validated by proof-of-concept: a unit test asserts `init_plan`'s decision (no flag -> none, flag with no repo -> init, flag with a `.git` present -> skip) without needing a git binary; and functional runs confirmed that `--init --write` creates `.git` in a fresh directory, a re-run over it skips, a non-terminal `--init` without `--write` previews the init line and writes nothing, and `--init --write` with `git` absent from `PATH` errors (exit 2) before any asset is written. 46 tests pass; clippy `-D warnings` clean. The fallback (raise rather than add a git-library dependency) was not needed.
