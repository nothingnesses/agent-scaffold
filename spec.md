...
Wrote the canonical `AGENTS.md` guidance (with a `{{principles}}` placeholder), the plan-document template, the three core prompts, and the principle data as structured TOML (initially 48 principles, 21 default; see `pack/principles.toml` for the current set, which has grown since), parsed and tested. Content policy: the shipped prompts and guidance stay principle-agnostic and defer to the user-selected principle set in `AGENTS.md`, rather than baking in specific opinions a user may not have chosen (the open-questions gate was corrected to follow this, and the prompts instruct the agent to read `AGENTS.md` to align with the current principles). The workflow (in `AGENTS.md`, supported by `adversarial-review.md`) is role-separated so no agent grades its own work: an orchestrator drives the phases and routes context; a planner drafts the plan; independent reviewers review adversarially (assume-issues, given the change context via before and after commit hashes or a diff range, with diverse lenses and models where available); a separate triager adjudicates each finding (never the agent that produced the artifact, and it may run a short producer-versus-reviewer debate on a contested finding); and an implementer makes the changes. Where the harness lacks sub-agents, one agent plays the roles in sequence but records each role's output. Relitigation is prevented by a transient review ledger (each finding, its verdict, reasoning, and action) that the orchestrator hands to each new round under a "no re-raise without new evidence" rule, with a bounded loop that escalates to a human if findings stay contested; the ledger is scratch state discarded at task close, kept out of the Open Questions section, and only durable decisions fold into the plan's steps (reopened only by evidence that beats the recorded reasoning). The plan template's Project Principles section (and the `AGENTS.md` Plan step that points at it) instructs the agent to seed a plan's principles from the `AGENTS.md` principles in order, add the project-specific ones after, consolidate any overlap into a single amended principle, and keep the list numbered so it can be referenced by number.
