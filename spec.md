...
Status: in progress. Name confirmed as `agent-scaffold`. Steps 1 to 5 are complete: the core assets, the file-dropper with two-tier ownership, the idempotency/safety pass, the full selection UI (non-interactive `--principles` with `default`/`all`/`none`/ids/`tag:`, plus the interactive two-pane ratatui TUI `--interactive` with `i`/`a` insert, `K`/`J` reorder, undo/redo, a `/` filter, and a save-confirmation modal), and Step 5's tag-based selection and filter (5d skipped by decision). No open questions remain. Steps 6 to 10 are optional extras. Step 6 (bring-your-own template) is the current work: its design is resolved (OQ-E/F/G adopted; OQ-H's git-URL fetch deferred to Step 10) and split into evidence-grounded sub-steps 6a and 6b. 6a is complete (the pack manifest, `include_dir` embedding, and the behaviour-preserving refactor of `assets()` into the `manifest` loader, validated byte-identical); 6b (external local-path packs via `--template` plus `--var`, with a `[[var]]` schema and built-in principles interim) is next to implement, followed by 6c (pack-owned principles). OQ-I and OQ-J are resolved and folded into 6b/6c. Steps 7 to 10 are not started (7 optional modules, 8 greenfield flake template, 9 later enhancements, 10 git-URL fetch). The implementation lives in the repo (`src/`, `pack/`); this spec is the durable context for resuming after a compaction. Verification convention: `cargo clippy --all-targets -- -D warnings`, `nix fmt`, and ASCII-clean before each commit.
...
Status: in progress (6a complete; 6b then 6c next). Support `--template <local-path>` so a project can scaffold from a user-supplied pack, not only the built-in one (git-URL fetch is deferred to Step 10; a Nix flake-ref stays out of scope). Reordered ahead of optional modules (Step 7) because it formalises the pack/manifest abstraction modules slot into.
...
Status: not started. Add `--template <path>` to load a pack from a filesystem directory through the same manifest loader, plus `--var key=value` (repeatable) and manifest-declared variables. Depends on 6a.
...
Status: not started. Make the principle set a property of the active pack: when `--template` is given, load the principle list from the pack source's `principles.toml` (falling back to an empty set when the pack ships none) and use it for selection, `--list-principles`, `--interactive`, and the `{{principles}}` value; with no `--template`, behaviour is unchanged (the built-in set). Depends on 6b (which threads the `PackSource` through `main`). The selection layer (`resolve_selection`, `run_selection`, `render_principles`) already takes the principle list as a parameter, so the change is confined to how `main` obtains that list. Adopted from OQ-J as the coherent end-state (a pack is self-contained, its principles included; Principle 1). Validate: a fixture pack shipping its own `principles.toml` renders and selects its principles (not the built-in ones) through `--template`, and the built-in path is unchanged (existing drop tests and the byte-identical golden still pass). Fallback: if reading principles from the pack source proves entangled with the built-in `default_principles` path, keep the built-in set for external packs (the 6b interim) and raise the entanglement.
