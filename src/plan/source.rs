...
enum_field! {
	...
	/// over from the Markdown Roadmap vocabulary. Defined via `enum_field!` (the same
	/// machinery as the `metrics` waiver enums, Principle 16) so its macro-generated
	/// `VARIANTS` list ties `ALL` to the full variant set: see the `const _` guard below.
	pub(crate) StepStatus {
		/// Not started.
...
impl StepStatus {
	...
	/// status distribution is deterministic (Principle 16, one ordering). The `const _`
	/// cross-check below ties this array's length to the macro-generated `VARIANTS`
	/// list, so adding a variant is a compile error until it is listed here.
...
/// Compile-time drift guard for `StepStatus::ALL`, mirroring `WaiverReason::ALL`:
/// `VARIANTS` is generated by the `enum_field!` macro from the full variant list, so
/// adding a `StepStatus` grows `VARIANTS` and this equality fails to compile until the
/// variant is also added to `ALL`. The tie is to `ALL` itself (its length), not merely
...
enum_field! {
	...
	/// `decided` status plus the typed `folded_into` cross-reference field. Defined via
	/// `enum_field!` (the same machinery as the `metrics` waiver enums, Principle 16) so
	/// its macro-generated `VARIANTS` list ties `ALL` to the full variant set: see the
	/// `const _` guard below.
	pub(crate) QuestionStatus {
...
impl QuestionStatus {
	...
	/// queue-status vocabulary fragment is deterministic. The `const _` cross-check
	/// below ties this array's length to the macro-generated `VARIANTS` list, so adding
	/// a variant is a compile error until it is listed here.
...
/// Compile-time drift guard for `QuestionStatus::ALL`, mirroring `WaiverReason::ALL`:
/// `VARIANTS` is generated by the `enum_field!` macro from the full variant list, so
/// adding a `QuestionStatus` grows `VARIANTS` and this equality fails to compile until
/// the variant is also added to `ALL`. The tie is to `ALL` itself (its length), not
/// merely to a match elsewhere, so a stale `ALL` that silently dropped the new status
/// from the render engine's generated queue-status vocabulary is a build error
/// (Principle 16).
const _: () = assert!(QuestionStatus::ALL.len() == QuestionStatus::VARIANTS.len());
